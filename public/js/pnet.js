...
function select_object(object)
{
    ...
    if (Pairing) {
        if (!Pair.from) {
            Pair.from = object;
            type_selected('['+Pair.from.key+',')
        } else {
            Pair.to = object;
            Pairing = false;   
            type_selected('['+Pair.from.key+','+Pair.to.key+']')    
            document.getElementById("btnAddArc").disabled = false;       
        }
    } 
}
...
function type_selected(text)
{
    ...
    if (selected_label != null) {
        selected_label.style.width = '120px';
...
{
    node_mouseup(this, e.shiftKey/*e.ctrlKey*/);
}
function transition_mouseup(e)
{
    node_mouseup(this, e.shiftKey/*e.ctrlKey*/);
}
function node_mouseup(node, key) {
    if (key && TEMP_ARC_ON) {
        TEMP_ARC.img.remove();
